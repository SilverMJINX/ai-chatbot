...
function stripBoilerplate(text: string): string {
  ...
  // Find content start — after *** START *** line
  ...
  // Find content end — before *** END *** line
...
function splitChapters(rawText: string): string[] {
  ...
  const chunks = text
    ...
    .map(c => c.trim())
    .filter(c => c.length > 80); // discard tiny heading-only fragments
...
function splitLines(section: string): string[] {
  ...
  return paragraphs.filter(p => {
    ...
    // Looks like an ALL-CAPS publisher/address line
    if (p === p.toUpperCase() && /[A-Z]{4,}/.test(p)) return false;
    // Starts with a number and is short — likely a page ref or address
          ...
          <>
            ...
            <div className="reader-tts-bar">
              ...
              <div className="reader-btn-row">
                <button className="chip chip-blue reader-ctrl-btn" onClick={() => skipLines(-5)} title="Back 5 paragraphs" type="button">⏮ 5</button>
                <button className="chip chip-blue reader-ctrl-btn" onClick={() => skipLines(-1)} title="Previous paragraph" type="button">‹ 1</button>
                ...
                <button className="chip chip-blue reader-ctrl-btn" onClick={() => skipLines(1)} title="Next paragraph" type="button">1 ›</button>
                <button className="chip chip-blue reader-ctrl-btn" onClick={() => skipLines(5)} title="Skip 5 paragraphs" type="button">5 ⏭</button>
            ...
            <div className="reader-body">
              {lines.length === 0 && (
                <p style={{ color: "#8aaac8", fontSize: 13, textAlign: "center", padding: "40px 0" }}>
                  No readable paragraphs found in this chapter.
                </p>
              )}
              {lines.map((line, i) => (
                <div
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .page-wrap { display: grid; grid-template-rows: 64px 1fr; min-height: 100vh; }
        ...
        .brand-name { font-family: 'Fraunces', Georgia, serif; font-size: 1.3rem; font-weight: 600; color: var(--text); letter-spacing: -0.02em; }
        .brand-name span { color: var(--blue); }
        .nav-divider { width: 1px; height: 24px; background: var(--border); margin: 0 4px; }
        .nav-badge { display: flex; align-items: center; gap: 6px; background: var(--blue-soft); color: var(--blue-dark); font-size: 0.72rem; font-weight: 600; padding: 5px 12px; border-radius: 20px; border: 1px solid var(--blue-mid); }
        .nav-books-link { font-size: 0.82rem; font-weight: 600; color: var(--white); text-decoration: none; padding: 6px 14px; border-radius: 20px; background: linear-gradient(135deg, var(--blue), #6baee8); border: 1.5px solid var(--blue); box-shadow: 0 2px 8px var(--blue-glow); white-space: nowrap; }
        .nav-chat-link { font-size: 0.82rem; font-weight: 600; color: var(--blue-dark); text-decoration: none; padding: 6px 14px; border-radius: 20px; background: var(--blue-soft); border: 1.5px solid var(--blue-mid); transition: all 0.15s; white-space: nowrap; }
        .nav-chat-link:hover { background: var(--blue-light); border-color: var(--blue); }
        .nav-spacer { flex: 1; }
        .nav-user { display: flex; align-items: center; gap: 10px; padding: 6px 12px; border-radius: 12px; cursor: pointer; transition: background 0.15s; }
        .nav-user:hover { background: var(--bg); }
        .nav-user-avatar { width: 32px; height: 32px; border-radius: 50%; background: linear-gradient(135deg, var(--blue), #6baee8); display: flex; align-items: center; justify-content: center; font-size: 0.78rem; font-weight: 700; color: white; box-shadow: 0 2px 8px var(--blue-glow); }
        .nav-user-name { font-size: 0.85rem; font-weight: 600; color: var(--text); }

        .books-main { max-width: 900px; width: 100%; margin: 0 auto; padding: 36px 24px; overflow-y: auto; }

        .books-subheader { display: flex; align-items: center; gap: 14px; padding-bottom: 24px; border-bottom: 1px solid var(--border); margin-bottom: 28px; }
        .ai-avatar-lg { width: 46px; height: 46px; border-radius: 14px; background: linear-gradient(135deg, var(--blue), #6baee8); display: flex; align-items: center; justify-content: center; box-shadow: 0 4px 14px var(--blue-glow); flex-shrink: 0; }
        ...
        .chip { font-size: 0.71rem; font-weight: 600; padding: 5px 11px; border-radius: 20px; border: 1.5px solid; white-space: nowrap; cursor: default; }
        ...
        .chip-speed-active { color: var(--blue-dark); background: var(--blue-light); border-color: var(--blue); font-weight: 700; }

        .search-wrapper { display: flex; align-items: center; gap: 10px; background: var(--white); border: 2px solid var(--border); border-radius: 20px; padding: 10px 10px 10px 20px; transition: border-color 0.2s, box-shadow 0.2s; box-shadow: var(--shadow-sm); margin-bottom: 28px; }
        .search-wrapper:focus-within { border-color: var(--blue); box-shadow: 0 0 0 4px rgba(74,144,217,0.12); }
        .search-input { flex: 1; border: none; outline: none; background: transparent; font-family: 'Plus Jakarta Sans', sans-serif; font-size: 0.9rem; color: var(--text); caret-color: var(--blue); }
        .search-input::placeholder { color: var(--text-soft); }

        .icon-btn { width: 38px; height: 38px; border-radius: 12px; border: none; cursor: pointer; display: flex; align-items: center; justify-content: center; transition: all 0.18s; flex-shrink: 0; }
        ...
        .send-btn:disabled { background: var(--blue-light); color: var(--text-soft); box-shadow: none; cursor: not-allowed; transform: none; }

        .books-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(220px, 1fr)); gap: 16px; }
        .book-card { background: var(--white); border: 1px solid var(--border); border-radius: 20px; padding: 18px; cursor: pointer; transition: all 0.18s; box-shadow: var(--shadow-sm); animation: fadeUp 0.28s cubic-bezier(0.22,1,0.36,1); }
        .book-card:hover { border-color: var(--blue); transform: translateY(-2px); box-shadow: 0 6px 20px rgba(74,144,217,0.15); }
        .book-card-title { font-size: 0.88rem; font-weight: 700; color: var(--text); margin-bottom: 5px; line-height: 1.4; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; }
        .book-card-author { font-size: 0.74rem; color: var(--text-soft); margin-bottom: 14px; display: -webkit-box; -webkit-line-clamp: 1; -webkit-box-orient: vertical; overflow: hidden; }
        ...
        .empty-state { text-align: center; padding: 60px 0; color: var(--text-soft); font-size: 0.9rem; }
        .empty-icon { font-size: 2.5rem; margin-bottom: 12px; }

        .typing-bubble { display: flex; align-items: center; gap: 5px; }
        .dot { width: 7px; height: 7px; border-radius: 50%; background: var(--blue); display: inline-block; animation: bounce 1.3s infinite ease-in-out; }
        .dot:nth-child(2) { animation-delay: 0.15s; }
        .dot:nth-child(3) { animation-delay: 0.3s; }

        .tts-btn { display: inline-flex; align-items: center; gap: 6px; font-size: 0.76rem; font-weight: 600; font-family: 'Plus Jakarta Sans', sans-serif; padding: 6px 14px 6px 11px; border-radius: 20px; border: 1.5px solid var(--blue-mid); background: var(--white); color: var(--blue-dark); cursor: pointer; transition: all 0.15s ease; white-space: nowrap; box-shadow: 0 1px 4px rgba(37,99,168,0.10); }
        .tts-btn:hover { background: var(--blue-soft); border-color: var(--blue); transform: translateY(-1px); box-shadow: 0 3px 10px rgba(74,144,217,0.2); }
        .tts-btn.tts-playing { background: #f0fdf4; border-color: #86efac; color: #166534; animation: ttsGlow 2s ease-in-out infinite; }
        .tts-btn.tts-loading { opacity: 0.7; cursor: wait; }
        .tts-label { font-size: 0.64rem; color: var(--text-soft); letter-spacing: 0.02em; }

        /* ── Reader modal ── */
        .reader-overlay { position: fixed; inset: 0; background: rgba(30,58,95,0.5); backdrop-filter: blur(6px); z-index: 100; display: flex; align-items: center; justify-content: center; padding: 24px; animation: fadeUp 0.2s ease; }
        .reader-panel { background: var(--white); border-radius: 20px; width: 100%; max-width: 740px; max-height: 88vh; display: flex; flex-direction: column; box-shadow: var(--shadow-md); overflow: hidden; border: 1px solid var(--border); }
        .reader-header { padding: 18px 20px; border-bottom: 1px solid var(--border); display: flex; align-items: center; justify-content: space-between; gap: 14px; flex-shrink: 0; }
        .reader-header-left { display: flex; align-items: center; gap: 14px; min-width: 0; }
        .reader-title { font-size: 0.95rem; font-weight: 700; color: var(--text); line-height: 1.3; display: -webkit-box; -webkit-line-clamp: 1; -webkit-box-orient: vertical; overflow: hidden; }
        .reader-author { font-size: 0.74rem; color: var(--text-soft); margin-top: 2px; }
        .reader-close-btn { width: 34px; height: 34px; border-radius: 10px; border: 1.5px solid var(--border); background: var(--bg); color: var(--text-mid); cursor: pointer; flex-shrink: 0; display: flex; align-items: center; justify-content: center; transition: all 0.15s; }
        .reader-close-btn:hover { background: var(--blue-light); border-color: var(--blue); color: var(--blue-dark); }

        .reader-tts-bar { padding: 12px 20px; border-bottom: 1px solid var(--border); display: flex; flex-direction: column; gap: 10px; flex-shrink: 0; }
        .reader-progress-row { display: flex; align-items: center; gap: 10px; }
        .reader-progress-track { flex: 1; height: 4px; background: var(--blue-mid); border-radius: 2px; overflow: hidden; }
        .reader-progress-fill { height: 100%; background: var(--blue); border-radius: 2px; transition: width 0.35s ease; }
        ...
        @keyframes ttsGlow { 0%, 100% { box-shadow: 0 0 0 0 rgba(134,239,172,0.4); } 50% { box-shadow: 0 0 0 6px rgba(134,239,172,0); } }
